...
- `web/src/cipher-suites.ts` provides `XChaCha20CipherWeb`: `XChaCha20Cipher`
  with a same-origin pool-worker override (mirrors `cli/src/cipher-suites.ts`).
  The default `blob:` worker is refused by Safari/WebKit under a strict CSP, so
  file transfer spawns a same-origin worker under `worker-src 'self'`. The
  worker file is generated by `web/build-worker.ts` into `web/public/` (served
  verbatim, never inlined) ahead of `dev`/`build`. Keep the CSP
  (`web/vite.config.ts`) and this override in lockstep; see
