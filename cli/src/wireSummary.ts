...
// Mirrors web/src/wireSummary.ts. Kept in sync manually: when a new wire
// message type or field is added there, mirror it here.
...
export function summarizeInbound(msg: InboundMsg): { summary: string; details: Record<string, string | number | boolean> } {
	switch (msg.type) {
	case 'room_created':
		return { summary: `room created ${msg.roomId}`,
	...
	case 'ratchet_step_fwd':
		return { summary: `${msg.from} ratchet step`,
			details: {
				from: msg.from,
				pn: msg.pn,
	...
	case 'ek_update_fwd':
		return { summary: `${msg.from} ek update`,
...
export function summarizeOutbound(msg: OutboundMsg): { summary: string; details: Record<string, string | number | boolean> } {
	...
	case 'join':
		return { summary: `join ${msg.roomId}`,
			details: { roomId: msg.roomId, roomSecret: redact(msg.roomSecret) } };
	case 'identify':
		return { summary: `identify as ${msg.username}`,
			details: { username: msg.username, ek: redact(msg.ek), ratchetEk: redact(msg.ratchetEk), claim: redact(msg.claim) } };
	case 'relay':
		return { summary: `relay self → ${msg.to}`,
