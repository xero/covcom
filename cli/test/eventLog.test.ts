...
// eventLog is a module-level singleton and Bun shares module state across test
// files. Other suites (e.g. state.test.ts) drive the state machine, which logs
// into this same buffer. Reset *before* each test too, so we never inherit
// events from a file that happened to run first (file order is sorted on macOS
// but arbitrary on Linux/CI).
