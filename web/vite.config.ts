...
//    eval/new Function. The crypto runs as WASM on the main thread and in pool
//    workers (which inherit this policy).
//  - worker-src/child-src blob: are REQUIRED: leviathan-crypto's SealStreamPool
//    spawns classic workers from blob: URLs (see its docs/architecture.md).
	...
	return {
		...
		generateBundle(_opts, bundle) {
			for (const file of Object.values(bundle)) {
				...
				// no external scripts, so no 'self' — strictest possible.
				const hashes: string[] = []
