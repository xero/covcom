...
import { PROTOCOL_HEX, VERSION } from './src/version.ts';

// Injects a <meta name="generator"> tag identifying the COVCOM version and
// wire protocol that produced this page. VERSION/PROTOCOL_HEX come from
// src/version.ts, regenerated by build.ts before each `vite build`. Runs in
// dev too, so the served page is inspectable either way.
function generator(): Plugin {
	return {
		name: 'covcom-generator',
		transformIndexHtml: () => [{
			tag: 'meta',
			attrs: { name: 'generator', content: `COVCOM v${VERSION} (protocol ${PROTOCOL_HEX})` },
			injectTo: 'head',
		}],
	};
}
